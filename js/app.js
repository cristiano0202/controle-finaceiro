const STORAGE_KEY = "controleFinanceiroDataV1";
...
function bindActions() {
  ...
  document.body.addEventListener("click", (event) => {
    ...
    if (button.dataset.togglePaid) {
      toggleBillStatus(button.dataset.id);
    }

    if (button.dataset.finishDebt) {
      finishDebt(button.dataset.id);
    }
  });
}
...
function bindForms() {
  document.getElementById("billForm").addEventListener("submit", (event) => {
    ...
    finishForm(event.currentTarget);
  });
